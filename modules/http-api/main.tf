...
resource "aws_apigatewayv2_integration" "this" {
  ...
  integration_uri = var.is_lambda_integration ? "arn:aws:apigateway:${local.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${local.region}:${data.aws_caller_identity.current.account_id}:function:${each.value.function_name}:$${stageVariables.lambdaAlias}/invocations" : each.value.uri
